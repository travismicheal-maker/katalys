// BioViseInfoPage.jsx
// Drop into src/ as BioViseInfoPage.jsx
// Props:
//   onLaunch — function called by "Use BioVise AI Now" button
...
export default function BioViseInfoPage({ onLaunch }) {
  ...
  return (
    <div style={{ fontFamily: 'Georgia, serif', color: C.text, background: C.offWhite, overflowY: 'auto', height: '100%' }}>
      ...
      <section style={{ background: C.emerald, color: C.white, padding: '72px 24px 64px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', maxWidth: 740, margin: '0 auto' }}>
          ...
          {onLaunch && (
            ...
            >
              Use BioVise AI Now <ArrowRight size={17} strokeWidth={2.5}/>
            </button>
          )}
        </div>
      </section>
        ...
        <>
          ...
          <div style={{ ...card({ background: C.mint, borderColor: C.mintDark }), display: 'flex', gap: 14, alignItems: 'flex-start' }}>
            ...
            <p style={{ margin: 0, fontSize: 13.5, color: C.emeraldMid, lineHeight: 1.65 }}>
              <strong>The core difference:</strong> When you ask ChatGPT about BPC-157, it searches the open web — including Reddit threads, supplement blogs, and bodybuilding forums. When you ask BioVise, it queries a curated clinical formulary built from peer-reviewed human and animal studies, then searches PubMed and Cochrane only if additional evidence is needed. The sources are not the same. The quality is not the same.
            </p>
          </div>
        </>,
        C.white
      ...
      {section(
        <>
          <SectionHeader
            ...
            sub="BioVise's retrieval chain is built around a single principle: clinical decisions require clinical-grade sources."
          />
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 14, marginBottom: 28 }}>
            <SourceTier label="Step 1 — Proprietary KB (always first)" sites={['BioVise Peptide Formulary', 'Bio Precision Aging Clinical Notes', 'Extracted Peer-Review Data']} color={C.emeraldMid} bg={C.mint} border={C.mintDark} />
          ...
          <div style={{ ...card({ background: C.redLt, borderColor: '#F5B8B8' }), marginBottom: 14, display: 'flex', gap: 14, alignItems: 'flex-start' }}>
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: 13, color: '#8B2020', lineHeight: 1.6 }}>
                WebMD · Healthline · Reddit · Mayo Clinic consumer pages · Bodybuilding.com · supplement retailer blogs · YouTube · any social media platform. These domains are blocked at the API proxy level — BioVise cannot retrieve from them even if prompted.
              </p>
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12 }}>
            {[
              ...
              { icon: Microscope, title: 'Statistics, not summaries',   body: 'BioVise presents actual study data: p-values, sample sizes, effect sizes, confidence intervals. "BPC-157 may help healing" becomes "83% tendon tensile strength recovery vs 30% control at day 14, p<0.001."' },
              { icon: BarChart3,  title: 'Bayesian clinical reasoning', body: 'BioVise reasons like a senior clinician — stating pre-test probability, updating on your uploaded records, and giving differential probabilities rather than binary yes/no answers.' },
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 14 }}>
            {[
              ...
              { icon: FileText,    title: 'Upload your hormone labs',          body: 'Upload testosterone panels, thyroid, estradiol, cortisol, DHEA, IGF-1, or any endocrine workup. BioVise auto-detects flagged values, compares them to guideline targets (Endocrine Society, AACE), and generates a clinical interpretation.', accent: C.gold },
              { icon: Search,      title: 'Guideline-anchored responses',      body: 'Every recommendation is anchored to current Endocrine Society, AACE, or relevant specialty guideline. BioVise states the guideline, the target, your lab value, and the gap — exactly the way a real consultant would.', accent: '#5B8DEF' },
              { icon: ShieldCheck, title: 'Drug interactions & safety alerts', body: 'BioVise proactively surfaces drug interactions, contraindications, and safety signals — not only when asked, but whenever the clinical picture warrants it. The same way a department chief would catch what the ordering physician missed.', accent: C.red },
      ...
      {section(
        <>
          <SectionHeader eyebrow="Records & Privacy" title="Your records are yours. Always." sub="BioVise handles your most sensitive medical documents. Here is exactly how it works." />
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 14 }}>
            {[
              ...
              { icon: ShieldCheck, title: 'Not used for AI training',       body: 'Unlike ChatGPT Plus, BioVise routes through an authenticated Anthropic API key with no data retention agreements. Your health data is not a training asset.' },
              ...
              { icon: FileText,    title: 'Paste text & voice input',       body: 'No PDF? Use the paste panel to copy-paste lab values, prescriptions, or clinical notes. Voice input lets you describe symptoms conversationally and BioVise responds with clinical-grade precision.' },
            ].map(f => <FeatureCard key={f.title} icon={f.icon} title={f.title}>{f.body}</FeatureCard>)}
          </div>
        </>,
        C.white
      ...
      {section(
        <>
          ...
          {[
            { q: 'Is BioVise a replacement for my doctor?',               a: "No. BioVise is a clinical decision-support and education platform. Every response ends with a clinical disclaimer. BioVise is designed to make you a more informed patient or a better-prepared clinician — not to replace the physician relationship." },
            { q: 'Where does the peptide knowledge come from?',           a: "The BioVise peptide knowledge base is built by a practicing peptide clinician who uploads peer-reviewed research articles directly into the system. Each article is read by Claude, which extracts dosing, mechanisms, side effects, evidence levels, and citations into structured entries in the proprietary formulary. This data is then injected into every peptide consultation as primary reference material." },
      ...
      <footer style={{ background: C.emerald, padding: '32px 24px', textAlign: 'center' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
          <img src={katalysLogo} alt="BioVise" style={{height:22,width:'auto'}}/>
          <span style={{ fontSize: 16, fontWeight: 700, color: C.white }}>BioVise Health</span>
        </div>
        <p style={{ fontSize: 12, color: 'rgba(255,255,255,.5)', margin: '0 auto', maxWidth: 520, lineHeight: 1.65 }}>
          For educational and clinical decision-support purposes only. All management decisions should be made in the context of the full clinical picture by the treating clinician. BioVise Health is not a licensed medical practice and does not establish a physician-patient relationship.
        </p>
