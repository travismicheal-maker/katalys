...
function ShareModal({ content, onClose }) {
  ...
  const socials = [
    ...
    { name:'X', bg:'#000', color:'#fff', emoji:'𝕏', url:`https://x.com/intent/tweet?text=${encodeURIComponent('Health insight from Katalys Health (powered by Bio Precision Aging): ' + shortText)}&url=${encodeURIComponent(window.location.href)}` },
    { name:'Reddit', bg:'#FF4500', color:'#fff', emoji:'R', url:`https://reddit.com/submit?url=${encodeURIComponent(window.location.href)}&title=${encodeURIComponent('Health insight from Katalys Health')}` },
    { name:'WhatsApp', bg:'#25D366', color:'#fff', emoji:'W', url:`https://wa.me/?text=${encodeURIComponent('Health insight from Katalys Health: ' + shortText + ' ' + window.location.href)}` },
  ];
  return (
...
function generatePDF(content, question) {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Katalys Health AI</title><style>
  @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@500;600;700&family=DM+Sans:opsz,wght@9..40,400;9..40,500;9..40,600&display=swap');
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  <div class="pdf-header"><div><div class="pdf-brand">Katalys Health AI</div><div class="pdf-brand-sub">Clinical Decision Support · Bio Precision Aging · bioprecisionaging.com</div></div><div><div class="pdf-meta-label">AI Clinical Summary</div><div class="pdf-meta-date">${now} · ${time}</div></div></div>
  ...
  <div>${bodyHTML}</div>
  <div class="pdf-footer"><div><div class="pdf-footer-disclaimer">This summary was generated by Katalys Health, a clinical decision-support tool powered by Claude AI and grounded in GRADE evidence methodology. For educational and clinical decision-support purposes only.</div><div class="pdf-footer-brand">Bio Precision Aging · bioprecisionaging.com</div></div><div class="pdf-footer-date">Generated ${now}</div></div>
  </body></html>`;
  ...
  return (
    <>
      ...
      <div className={isMobile?'mob-msgs':'desk-msgs'}>
        {(msgs||[]).map((m,i)=>(
          <div key={i} className={`${isMobile?'msg':'desk-msg'} ${m.role==='user'?'u':'a'} fu`}>
            <div className="mrole" style={{display:'flex',alignItems:'center',gap:6}}>{m.role==='user'?'You':'Katalys'}{m.role==='assistant'&&m._model?.includes('opus')&&(<span className="model-badge badge-opus"><Brain size={9}/>Deep analysis · Opus</span>)}{m.role==='assistant'&&m._model&&!m._model.includes('opus')&&(<span className="model-badge badge-sonnet"><Zap size={9}/>Sonnet</span>)}</div>
...
export default function KatalysHealth() {
  ...
  const NAV=[{id:'home',lbl:'Home',I:Home},{id:'records',lbl:'Records',I:FolderOpen},{id:'ai',lbl:'AI Consultant',I:MessageSquare},{id:'peptide',lbl:'Peptide Consultant',I:Dna},{id:'hormone',lbl:'Hormone Consultant',I:Brain},{id:'info',lbl:'What is Katalys?',I:Info},{id:'plans',lbl:'Plans',I:CreditCard},{id:'profile',lbl:'Profile',I:User}];
          ...
          <div className="modal" style={{maxWidth:420}}>
            ...
            <div className="modal-body">
              <div style={{textAlign:'center',padding:'8px 0 16px'}}>
                <div style={{width:52,height:52,borderRadius:'50%',background:'#C8DFF0',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 14px'}}><Heart size={22} color="#1C3D5A" fill="#6B9EC8"/></div>
      ...
      <div className="mob-wrap">
        <div className="phone">
          <div className="mob-hd">
            {page==='home'
              ?<div className="logo"><img src="/katalys-logo.PNG" alt="Katalys" style={{height:22,width:'auto'}}/>Katalys Health<span style={{fontSize:11,fontWeight:400,color:'var(--mu)',borderLeft:'1px solid var(--bd)',paddingLeft:8,marginLeft:2}}><a href="https://www.bioprecisionaging.com" target="_blank" rel="noopener noreferrer" style={{color:'var(--mu)',textDecoration:'none'}}>Bio Precision Aging</a></span></div>
      ...
      <div className="desk-app">
        ...
        <main className="desk-main">
          <div className="desk-topbar">
            <div>
              <div className="desk-page-title">{page==='home'?`Good morning`:{records:'My Records',ai:'AI Consultant',peptide:'Peptide Consultant',hormone:'Hormone Consultant',info:'What is Katalys?',plans:'Plans',profile:'Profile'}[page]}</div>
