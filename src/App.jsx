...
import BioViseInfoPage from './BioViseInfoPage';
...
// ── Main App ──────────────────────────────────────────────────────────────────
export default function BioViseHealth() {
  ...
  const [freeCount,setFreeCount]=useState(()=>parseInt(localStorage.getItem('biovise_free')||'0'));
  ...
  const send=async(text)=>{
    ...
    const userTier=localStorage.getItem('biovise_tier')||'explorer';
    if(userTier==='explorer'&&freeCount>=3){setShowUpgrade(true);return;}
    if(userTier==='explorer'){const next=freeCount+1;setFreeCount(next);localStorage.setItem('biovise_free',next);}
    const h=[...(msgs||[]),{role:'user',content:m}];setMsgs(h);setInput('');setBusy(true);
    try{
    ...
    <>
      ...
      <div className="mob-wrap">
        <div className="phone">
          ...
          <div className="mob-content">
            ...
            {page==='info'&&<div style={{overflowY:'auto',height:'100%',flex:1}}><BioViseInfoPage onLaunch={()=>setPage('home')}/></div>}
            {page==='plans'&&<div className="mob-pad"><PricingPage onSelectPlan={(plan)=>{toast2(`Redirecting to ${plan} checkout…`);}} currentTier={localStorage.getItem('biovise_tier')||'explorer'}/></div>}
      ...
      <div className="desk-app">
        ...
        <main className="desk-main">
          ...
          {page==='info'&&<div style={{flex:1,overflowY:'auto',minHeight:0}}><BioViseInfoPage onLaunch={()=>setPage('home')}/></div>}
          {page==='plans'&&<div className="desk-content"><PricingPage onSelectPlan={(plan)=>{toast2(`Redirecting to ${plan} checkout…`);}} currentTier={localStorage.getItem('biovise_tier')||'explorer'}/></div>}
